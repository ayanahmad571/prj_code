...
for path in paths:
    ...
    with open(p+path) as file:
        ...
        for row in data:
            ...
            if(type(tweet_obj) != NoneType):
                t_hashtags = 0
                t_urls = 0
                t_mentions = 0
                t_words = 0
                t_posts = len(tweet_obj)
                for u_tweet in tweet_obj:
                    i_hashtags = u_tweet.count("#")

                    i_urls_http = u_tweet.count("http://")
                    i_urls_https = u_tweet.count("https://")
                    i_urls = i_urls_http + i_urls_https
                    
                    i_mentions = u_tweet.count("@")
                    i_words = len(u_tweet.split())
                    
                    #add to master values
                    t_hashtags += i_hashtags
                    t_urls += i_urls
                    t_mentions += i_mentions
                    t_words += i_words

                hashtags_per_post             = t_hashtags/t_posts
                urls_per_post                 = t_urls/t_posts
                mentions_per_post             = t_mentions/t_posts
                words_per_post                = t_words/t_posts
